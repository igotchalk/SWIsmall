...
class GlobalSkyTEM(GlobalAEM):
    ...
    def sigma_padded(self):
        if getattr(self, '_sigma_padded', None) is None:
            ...
            # print ('kang', index_at_topo_up.shape)
            # print ('kang', index_at_topo.shape)
    ...
    def write_inputs_on_disk(self, i_src):
        ...
        dill.dump(
            storage, open(Path(self.work_dir).joinpath("inputs_{}.pkl".format(i_src)).as_posix(), 'wb')
        )
