...
'''
Differences with unconfined simulation:
1) Top of model is at -20 m instead of 3 m
2) LPF laytyp=0 (confined) instead of 1 (convertible)
3) Recharge turned off
4) Farm recharge turned off
5) river recharge turned off
...
swexe = config.swexe
it = 501
MC_file = Path('../work/heterog_1000/MC_expt_2020-02-11-confined/expt.txt')
...
def update_run_model(varlist, it, m=m, homogenous=2, runyn=True,
    ...
    ''' REMOVED FOR CONFINED
    # flopy.modflow.ModflowRch(m, rech=rech_data)
    # flopy.modflow.ModflowRiv(m, stress_period_data=riv_data)
    '''
    [m.remove_package(p) for p in m.get_package_list() if p in ('RIV','RCH')]
	# Add LPF package to the MODFLOW model
    flopy.modflow.ModflowLpf(m, hk=hk_grid, vka=vka, ipakcb=ipakcb, laytyp=0,laywet=0,ss=ss,sy=sy)
